...
Route::middleware(['suspendue'])->group(function () {
    Route::middleware(['non_valide'])->group(function () {
        ...
        Route::middleware(['admin'])->group(function () {
            ...
            Route::post('/recherche_user', [App\Http\Controllers\User_Controller::class,'Recherche_user'])->name('recherche_user');
            Route::get('/edit_user/{id}', [App\Http\Controllers\User_Controller::class,'edit_user'])->name('edit_user');
            Route::put('/update_user/{id}', [App\Http\Controllers\User_Controller::class,'update_user'])->name('update_user');

            Route::post('/update-suspendue/{id}/{status}', [App\Http\Controllers\User_Controller::class, 'updatesuspendue'])->name('updatesuspendue');
        });
    });
});
Route::get('/non_valide', [App\Http\Controllers\Controller::class,'non_valide'])->name('non_valide')->middleware("auth");
Route::get('/suspendue', [App\Http\Controllers\Controller::class,'suspendue'])->name('suspendue')->middleware("auth");
